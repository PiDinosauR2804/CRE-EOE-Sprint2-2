...
class BaseData:
    ...
    def filter_and_add_desciption_and_2_old_description(self, labels, descriptions, seen_labels, old_descriptions):
        if not isinstance(labels, list):
            labels = [labels]
        print(labels)
        res = []
        ...
        for label in labels:
            ...
            for anchor in self.train_data[label]:
                ...
                if lenght_seen_labels != 0:
                    ...
                    old_labels_1 = seen_labels[(count_negative_label//lenght_seen_labels)%lenght_seen_labels]
                    # ins.update({
                    #         'old_labels': self.label2id[old_labels]
                    #     })
                    old_pools_1 = {}
                    if old_labels_1 in old_descriptions.keys():
                        old_pools_1 = old_descriptions[old_labels_1]
                        
                    for idx, pool in enumerate(old_pools_1):
                        ins.update({
                            f'old_description_1_ids_{idx}': pool
                        })
